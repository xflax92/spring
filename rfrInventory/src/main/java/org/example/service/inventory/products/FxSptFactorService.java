...
@Service
public class FxSptFactorService extends FactorService {

    private String assetClassService = Const.FX;
    private String factorTypeService = Const.SPT;
    ...
    public List<Factor> getInventorySCM(InventoryRequest req){
        RfInfo rfInfo = new RfInfo("FX", "SPT", "SCM");
    ...
    public List<Factor> getInventoryOPE(InventoryRequest req){
        RfInfo rfInfo = new RfInfo("FX", "SPT", "OPE");
        Factor factor = new Factor(new HeaderCtrl(), rfInfo, new RfBody());
        return Arrays.asList(factor);
    }

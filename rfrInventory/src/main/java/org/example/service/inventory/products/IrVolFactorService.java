...
@Service
public class IrVolFactorService extends FactorService {

    private String assetClassService = Const.IR;
    private String factorTypeService = Const.VOL;
    ...
    public List<Factor> getInventorySCM(InventoryRequest req){
        RfInfo rfInfo = new RfInfo("IR", "VOL", "SCM");
    ...
    public List<Factor> getInventoryOPE(InventoryRequest req){
        RfInfo rfInfo = new RfInfo("IR", "VOL", "OPE");
        Factor factor = new Factor(new HeaderCtrl(), rfInfo, new RfBody());
        return Arrays.asList(factor);
    }
